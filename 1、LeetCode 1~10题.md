...
10. 正则表达式匹配
    ...
    如果p[j+1] 是*的话，那么p[j]需要和p[j+1]联合起来使用，p[j]不能单独做处理，因此跳过；

    如果p[j]不是*的话，那么p[j]需要和s[i]匹配或者p[j] == '.'；

    如果p[j]是*的话，那么需要看p[j-1]p[j]它们匹配s串的几个字符：0个、1个、2个……
